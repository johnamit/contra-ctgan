...
def main():
    ...
    clambda_str = str(clambda).replace('.', '_')
    ...
    model_path = f"Models/ContrastiveCTGAN_lambda{clambda_str}_temp{temp_str}.pth"
    ...
    synthetic_data_path = f'SyntheticDatasets/synthetic_data_ContrastiveCTGAN_lambda{clambda_str}_temp{temp_str}.csv'
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f'Plots/loss_curves_ContrastiveCTGAN_lambda{clambda_str}_temp{temp_str}.png')
    plt.show()
